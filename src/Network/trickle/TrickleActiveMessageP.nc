generic module TrickleActiveMessageP() {
...
uint8_t getTrickleType(message_t* msg) {
	uint8_t t;
	uint8_t *ptr = (uint8_t*)call MacAMSend.getPayload(msg, sizeof(2));
	if (ptr == NULL) {
		dbg("Network", "TrickleActiveMessageP getTrickleType(0x%1x) - got NULL", msg);
		return 0;
	}
	ptr++;
	t = *ptr;
...
void do_sendDone(message_t *msg, error_t error) {
	dbg("Network", "TrickleActiveMessageP do_sendDone(0x%1x, %d) - type %d",
				msg, error, getTrickleType(msg));
	signal AMSend.sendDone[getTrickleType(msg)](msg, error);
}

message_t* do_receive(message_t *msg, void *payload, uint8_t len) {
	dbg("Network", "TrickleActiveMessageP do_receive(0x%1x, 0x%1x, %d) - type %d",
				msg, payload, len, getTrickleType(msg));
	return signal Receive.receive[getTrickleType(msg)](msg, (void*)(((uint8_t*)payload)), len);
}

message_t* do_snoop(message_t *msg, void *payload, uint8_t len) {
	dbg("Network", "TrickleActiveMessageP do_snoop(0x%1x, 0x%1x, %d) - type %d",
				msg, payload, len, getTrickleType(msg));
	return signal Snoop.receive[getTrickleType(msg)](msg, (void*)(((uint8_t*)payload)), len);
}
