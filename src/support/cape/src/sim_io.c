...
int sim_add_read_io(uint16_t node_id, uint8_t io_size)__attribute__ ((C, spontaneous)) {
	return 0;
}

int sim_add_write_io(uint16_t node_id, uint8_t io_size)__attribute__ ((C, spontaneous)) {
	return 0;
}
