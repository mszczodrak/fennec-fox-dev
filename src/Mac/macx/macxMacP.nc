...
command error_t SplitControl.start() {
	dbg("Mac", "macxMac SplitControl.start()");
	post startDone_task();
...
command error_t SplitControl.stop() {
	dbg("Mac", "macxMac SplitControl.stop()");
	shutdown();
...
//	dbg("Mac", "macxMac RadioControl.startDone(%d)", err);
}


event void RadioControl.stopDone(error_t err) {
//	dbg("Mac", "macxMac RadioControl.stopDone(%d)", err);
} 
...
command error_t MacAMSend.send(am_addr_t addr, message_t* msg, uint8_t len) {
	...
	dbg("Mac", "macxMac MacAMSend.send(%d, 0x%1x, %d )", addr, msg, len);
...
event message_t* RadioReceive.receive(message_t* msg, void* payload, uint8_t len) {
	...
	if(!(metadata)->crc) {
		dbg("Mac", "macxMac MacAMSend.receive did not pass CRC");
		return msg;
	}
...
//    msg->conf = call MacAMPacket.group(msg);
	...
	if (call MacAMPacket.isForMe(msg)) {
	        dbg("Mac", "macxMac MacReceive.receive(0x%1x, 0x%1x, %d )", msg,
		...
		dbg("Mac", "macxMac MacSnoop.receive(0x%1x, 0x%1x, %d )", msg,
			ptr + sizeof(fennec_header_t),
			len - sizeof(fennec_header_t));
...
async event void RadioBuffer.loadDone(message_t* msg, error_t error) {
	dbg("Mac", "macxMac MacAMSend.loadDone(0x%1x, %d )", msg, error);
...
async event void RadioSend.sendDone(message_t *msg, error_t error) {
	dbg("Mac", "macxMac MacAMSend.sendDone(0x%1x, %d )", msg, error);
	m_state = S_STARTED;
