...
implementation {
  ...
  command error_t Mgmt.start() {
    ...
    if ((TOS_NODE_ID == call phidgetLightAndMotionAppParams.get_destination()) || 
	        (NODE == call phidgetLightAndMotionAppParams.get_destination())) {
  ...
  void prepare_network_message(uint8_t id) {
    ...
    q.addr = call phidgetLightAndMotionAppParams.get_destination();
  ...
  task void setup_app() {
    ...
    sensors[0].sample_count = call phidgetLightAndMotionAppParams.get_s1_sampleCount();
    sensors[0].freq = call phidgetLightAndMotionAppParams.get_s1_freq();
    sensors[0].seqno = 0;
    sensors[0].msg = NULL;
    call Sensor_0_Ctrl.set_rate(sensors[0].freq);
    call Sensor_0_Ctrl.set_signaling(TRUE);
    call Sensor_0_Setup.set_input_channel(call phidgetLightAndMotionAppParams.get_s1_inputChannel());

    sensors[1].sample_count = call phidgetLightAndMotionAppParams.get_s2_sampleCount();
    sensors[1].freq = call phidgetLightAndMotionAppParams.get_s2_freq();
    sensors[1].seqno = 0;
    sensors[1].msg = NULL;
    call Sensor_1_Ctrl.set_rate(sensors[1].freq);
    call Sensor_1_Ctrl.set_signaling(TRUE);
    call Sensor_1_Setup.set_input_channel(call phidgetLightAndMotionAppParams.get_s2_inputChannel());
