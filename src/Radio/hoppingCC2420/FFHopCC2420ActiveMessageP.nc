...
generic module FFHopCC2420ActiveMessageP(am_addr_t sink) @safe() {
...
implementation {
  ...
  command am_group_t AMPacket.localGroup() {
    return call HopCC2420Config.getPanAddr();
  }
  ...
  event void SubSend.sendDone(message_t* msg, error_t result) {
    if ((call CC2420PacketBody.getMetadata( msg ))->ack) {
      dbgs(F_RADIO, S_NONE, DBGS_ACKED, 0, 0);
    } else {
      dbgs(F_RADIO, S_NONE, DBGS_NOT_ACKED, 0, 0);
    }


    call HopCC2420Config.nextChannel();
