...
generic module FFHopCC2420ReceiveP(uint16_t sink, bool speedup_receive) @safe() {
...
implementation {
  ...
  void dec(){
    ...
    if( (header.fcf & (1 << IEEE154_FCF_SECURITY_ENABLED)) && (crc << 7) ){
      if(call HopCC2420Config.isAddressRecognitionEnabled()){
	if(!(header.dest==call HopCC2420Config.getShortAddr() || header.dest==AM_BROADCAST_ADDR)){
	  packetLength = header.length + 1;
	  m_state = S_RX_LENGTH;
	  call SpiResource.release();
	  beginReceive();
  ...
  async event void RXFIFO.readDone( uint8_t* rx_buf, uint8_t rx_len,
    ...
    case S_RX_FCF:
       ...
       */
      if(call HopCC2420Config.isAutoAckEnabled() && !call HopCC2420Config.isHwAutoAckDefault()) {
        if (((( header->fcf >> IEEE154_FCF_ACK_REQ ) & 0x01) == 1)
            && ((header->dest == call HopCC2420Config.getShortAddr())
  ...
  /****************** HopCC2420Config Events ****************/
  event void HopCC2420Config.syncDone( error_t error ) {
  }
  ...
  bool passesAddressCheck(message_t *msg) {
    ...
    if (mode == IEEE154_ADDR_SHORT) {
      return (header->dest == call HopCC2420Config.getShortAddr()
              || header->dest == IEEE154_BROADCAST_ADDR);
    } else if (mode == IEEE154_ADDR_EXT) {
      ieee_eui64_t local_addr = (call HopCC2420Config.getExtAddr());
