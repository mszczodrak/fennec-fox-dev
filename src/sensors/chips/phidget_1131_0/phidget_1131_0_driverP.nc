...
module phidget_1131_0_driverP @safe() {
...
implementation {
  ...
  uint16_t calibrated_data[PHIDGET_1131_0_SENSOR_HIST_LEN] = {0};
  uint8_t index = 0;

  norace bool signaling = PHIDGET_1131_0_DEFAULT_SIGNALING;
  ...
  command error_t SensorCtrl.start() {
    ...
    if (adc_channel_set == FALSE) {
      call SubAdcSetup.set_input_channel(PHIDGET_1131_0_DEFAULT_ADC_CHANNEL);
    }
...
command sensor_type_t SensorInfo.getType() {
        return F_SENSOR_THIN_FORCE;
}

command sensor_id_t SensorInfo.getId() {
        return FS_PHIDGET_1131_0;
}
  ...
  event void AdcSensorRaw.readDone(error_t error, uint16_t data) {
    if (error == SUCCESS) {
      /* No calibration for phidget_1131_0 */
      index++;
      index %= PHIDGET_1131_0_SENSOR_HIST_LEN;
