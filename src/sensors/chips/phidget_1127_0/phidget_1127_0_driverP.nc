...
implementation {
  ...
  uint16_t calibrated_data[PHIDGET_1127_0_SENSOR_HIST_LEN] = {0};
  uint8_t index = 0;

  norace bool signaling = PHIDGET_1127_0_DEFAULT_SIGNALING;
  ...
  command error_t SensorCtrl.start() {
    ...
    if (adc_channel_set == FALSE) {
      call SubAdcSetup.set_input_channel(PHIDGET_1127_0_DEFAULT_ADC_CHANNEL);
    }
...
command sensor_id_t SensorInfo.getId() {
  ...
  event void AdcSensorRaw.readDone(error_t error, uint16_t data) {
    if (error == SUCCESS) {
      ...
      index++;
      index %= PHIDGET_1127_0_SENSOR_HIST_LEN;
