...
implementation {
  ...
  uint16_t calibrated_data[PHIDGET_1133_0_SENSOR_SOUND_HIST] = {0};
  uint8_t index = 0;
  bool occurence_data = 0;

  uint16_t sensitivity = PHIDGET_1133_0_DEFAULT_SENSITIVITY;
  uint32_t rate = PHIDGET_1133_0_DEFAULT_RATE;
  uint8_t signaling = PHIDGET_1133_0_DEFAULT_SIGNALING;
  ...
  event void Resource.granted() {
    call Msp430Adc12SingleChannel.configureSingle(&phidget_1133_0_adc_config);
    call Msp430Adc12SingleChannel.getData();
  }

  task void check_event() {
    uint16_t delta = sensitivity * PHIDGET_1133_0_SENSOR_SOUND_STEP;

    uint32_t avg = 0;
    uint8_t i = 0;
    for (i = 0; i < PHIDGET_1133_0_SENSOR_SOUND_HIST; i++) {
      if (i != index) avg += calibrated_data[i];
    }

    avg /= (PHIDGET_1133_0_SENSOR_SOUND_HIST - 1);
  ...
  task void calibrate() {
    /* Calibration for phidget_1133_0 */
    
    index++;
    index %= PHIDGET_1133_0_SENSOR_SOUND_HIST;

    calibrated_data[index] = raw_data * 16.801;
