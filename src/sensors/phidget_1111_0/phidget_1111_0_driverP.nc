...
implementation {
  ...
  uint16_t sensitivity = PHIDGET_1111_0_DEFAULT_SENSITIVITY;
  uint32_t rate = PHIDGET_1111_0_DEFAULT_RATE;
  uint8_t signaling = PHIDGET_1111_0_DEFAULT_SIGNALING;
  ...
  event void Resource.granted() {
    call Msp430Adc12SingleChannel.configureSingle(&phidget_1111_0_adc_config);
    call Msp430Adc12SingleChannel.getData();
  }

  task void check_event() {
    uint16_t delta = sensitivity * PHIDGET_1111_0_SENSOR_MOTION_STEP;

    if ((calibrated_data < (PHIDGET_1111_0_SENSOR_NO_MOTION - delta)) || 
	(calibrated_data  > (PHIDGET_1111_0_SENSOR_NO_MOTION + delta))) {
      occurence_data = 1;
    } else {
      occurence_data = 0;
    }
